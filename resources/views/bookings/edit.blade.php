...
<div class="page-content">
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form action="{{ route('bookings.update', $booking->id) }}" method="POST" onsubmit="return validateForm()">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    ...
                    <select name="screening_id" class="form-control" id="screening_id" required>
                        ...
                        @foreach($screenings as $screening)
                            <option value="{{ $screening->id }}"
                                {{ $screening->id == $booking->screening_id ? 'selected' : '' }}>
                                {{ $screening->film->title }} ({{ $screening->show_time }})
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="seats_booked" class="form-control" id="seats_booked" min="1" required readonly value="{{ count($booking->seats) }}">
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="hidden" name="seats[]" id="selected-seats" value="{{ json_encode($booking->seats->pluck('seat_number')->toArray()) }}">
                </div>
        ...
        if (availableSeats && availableSeats[screeningId]) {
            ...
            seatsByRow.forEach((rowSeats, rowIndex) => {
                ...
                rowSeats.forEach((seat) => {
                    ...
                    rowDiv.appendChild(seatButton); // Tambahkan tombol kursi ke baris
                });
